 ...
 * but instead through the child classes of CountryAddForm and CountryEditForm.
 *
 * @ingroup country
 */
class CountryFormBase extends EntityForm {
  ...
  protected $continent_storage;

  /**
   * Construct the CountryFormBase.
   *
   ...
   *   An entity query factory for the robot entity type.
   */
  public function __construct(EntityStorageInterface $entity_storage, EntityStorageInterface $continent_storage) {
    $this->entityStorage = $entity_storage;
    $this->continent_storage = $continent_storage;
  }

  /**
   * Factory method for CountryFormBase.
   *
  ...
  public static function create(ContainerInterface $container) {
    $form = new static($container->get('entity_type.manager')->getStorage('country'),
                       $container->get('entity_type.manager')->getStorage('continent'));
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // annotation on our Country class.
    $country = $this->entity;
    ...
    $continents = $this->continent_storage->loadMultiple();
    foreach ($continents as $key => $continent) {
      $continent_keys[$key] = $this->t("@title", [
    ...
    $form['continent_id'] = [
      ...
      '#default_value' => $country->continent_id,
      '#options' => $continent_keys,
      '#attributes' => ['class' => ['continents']],
      '#required' => TRUE,
    ];
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status == SAVED_UPDATED) {
      ...
      $this->messenger()->addMessage($this->t('Country %label has been updated.', ['%label' => $country->name]));
      $this->logger('contact')->notice('Country %label has been updated.', ['%label' => $country->name, 'link' => $edit_link]);
    }
    else {
      // If we created a new entity...
      $this->messenger()->addMessage($this->t('Country %label has been added.', ['%label' => $country->name]));
      $this->logger('contact')->notice('Country %label has been added.', ['%label' => $country->name, 'link' => $edit_link]);
    }

    // Redirect the user back to the listing route after the save operation.
    $form_state->setRedirect('entity.country.list');
  }
