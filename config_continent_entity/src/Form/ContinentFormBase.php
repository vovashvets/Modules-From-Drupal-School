 ...
 * but instead through the child classes of ContinentAddForm and ContinentEditForm.
 *
 * @ingroup continent
 */
class ContinentFormBase extends EntityForm {
  ...
  /**
   * Construct the ContinentFormBase.
   *
  ...
  /**
   * Factory method for ContinentFormBase.
   *
  ...
  public static function create(ContainerInterface $container) {
    $form = new static($container->get('entity_type.manager')->getStorage('continent'));
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    // annotation on our Continent class.
    $continent = $this->entity;

    // Build the form.
    $form['id'] = [
      '#type' => 'machine_name',
      '#title' => $this->t('Machine name'),
      '#default_value' => $continent->id(),
      '#machine_name' => [
        'exists' => [$this, 'exists'],
        'replace_pattern' => '([^a-z0-9_]+)|(^custom$)',
        'error' => 'The machine-readable name must be unique, and can only contain lowercase letters, numbers, and underscores. Additionally, it can not be the reserved word "custom".',
      ],
      '#disabled' => !$continent->isNew(),
    ];
    $form['name'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Name'),
      '#maxlength' => 255,
      '#default_value' => $continent->name,
      '#required' => TRUE,
    ];
    $form['code'] = [
      '#type' => 'textfield',
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    $continent = $this->getEntity();

    // Drupal already populated the form values in the entity object. Each
    // form field was saved as a public variable in the entity class. PHP
    // allows Drupal to do this even if the method is not defined ahead of
    // time.
    $status = $continent->save();
    ...
    if ($status == SAVED_UPDATED) {
      ...
      $this->messenger()->addMessage($this->t('Continent %label has been updated.', ['%label' => $continent->name]));
      $this->logger('contact')->notice('Continent %label has been updated.', ['%label' => $continent->name, 'link' => $edit_link]);
    }
    else {
      // If we created a new entity...
      $this->messenger()->addMessage($this->t('Continent %label has been added.', ['%label' => $continent->name]));
      $this->logger('contact')->notice('Continent %label has been added.', ['%label' => $continent->name, 'link' => $edit_link]);
    }

    // Redirect the user back to the listing route after the save operation.
    $form_state->setRedirect('entity.continent.list');
  }
